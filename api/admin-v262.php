...
const CURRENT_ANALYTICS_APP_VERSION = 'v2-prod-20260912-conversion';
...
function audit_admin_token(string $password): string
{
    return hash_hmac('sha256', ADMIN_TOKEN_MESSAGE, $password);
}

function audit_admin_authorized(string $password): bool
{
    $cookie = (string)($_COOKIE[ADMIN_COOKIE] ?? '');
    return $cookie !== '' && hash_equals(audit_admin_token($password), $cookie);
}

function audit_int_fields(array $row, array $keys): array
{
    foreach ($keys as $key) $row[$key] = (int)($row[$key] ?? 0);
    return $row;
}

function audit_add_conversion(array $row): array
{
...
function audit_scope_clause(PDO $pdo, string $scope, string $alias = ''): string
{
    if ($scope !== 'current') return '1=1';
    $prefix = $alias !== '' ? $alias . '.' : '';
    return $prefix . 'app_version = ' . $pdo->quote(CURRENT_ANALYTICS_APP_VERSION);
}

function audit_diagnostics_for_window(PDO $pdo, string $where): array
{
    $where = "({$where}) AND app_version = " . $pdo->quote(CURRENT_ANALYTICS_APP_VERSION);
...
function audit_diagnostics_data(PDO $pdo): array
{
    return [
        'app_version' => CURRENT_ANALYTICS_APP_VERSION,
        'today' => audit_diagnostics_for_window($pdo, 'created_at >= CURDATE()'),
        'seven_days' => audit_diagnostics_for_window($pdo, 'created_at >= CURDATE() - INTERVAL 6 DAY'),
    ];
}

function audit_flow_details(PDO $pdo): array
{
    $version = $pdo->quote(CURRENT_ANALYTICS_APP_VERSION);
    $flowSql = "
        SELECT flow_id,
    ...
    foreach ($flowRows as $flowRow) {
        ...
        $eventStmt->execute([$flowId, CURRENT_ANALYTICS_APP_VERSION]);
...
function audit_dashboard_data(PDO $pdo, string $scope): array
{
    $scope = $scope === 'all' ? 'all' : 'current';
    $scopeClause = audit_scope_clause($pdo, $scope);
    $flowStartScope = audit_scope_clause($pdo, $scope, 'flow_start_event');
    $flowEventScope = audit_scope_clause($pdo, $scope, 'flow_event');
    ...
    ";
    $today = audit_int_fields($pdo->query($todaySql)->fetch() ?: [], [
    ...
    ";
    $today = array_merge($today, audit_int_fields($pdo->query($visitorTypeSql)->fetch() ?: [], ['new_visitors', 'returning_visitors']));
    ...
    ";
    $sources = array_map(static fn(array $row): array => audit_add_conversion([
    ...
    ";
    $devices = array_map(static fn(array $row): array => audit_add_conversion([
    ...
    $growthSql = "
        ...
        WHERE created_at >= CURDATE() - INTERVAL 29 DAY AND {$scopeClause}
    ";
    $growth = audit_int_fields($pdo->query($growthSql)->fetch() ?: [], [
    ...
    ";
    $total = audit_int_fields($pdo->query($totalSql)->fetch() ?: [], ['visitors','page_views','result_visitors']);
    ...
    return [
        ...
        'scope' => $scope, 'app_version' => CURRENT_ANALYTICS_APP_VERSION, 'analytics_version' => 'a5', 'generated_at' => date('Y-m-d H:i:s'),
    ];
}

$method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET'));
if ($method !== 'GET') respond(['ok' => false, 'error' => 'method_not_allowed'], 405);
if (!audit_admin_authorized($adminPassword)) respond(['ok' => false, 'error' => 'unauthorized'], 401);
...
try {
    respond([
        'ok' => true,
        'dashboard' => audit_dashboard_data($pdo, $scope),
        'diagnostics' => audit_diagnostics_data($pdo),
        'audit' => audit_flow_details($pdo),
    ]);
