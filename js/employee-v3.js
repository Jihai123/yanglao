import {
  ...
  claimDateFromAge,
  parseMonth,
} from './policy.js?v=20260828-p15';
import { projectPlanV3 } from './projection-v3.js?v=20260828-p15';
...
const state = {
  ...
  historyMode: 'quick', historyPattern: 'unknown', historySegments: [], avgIndex: 1,
...
function futureContributionSegments(category = mapCategory()) {
  ...
  if (state.intent === 'normal') {
    return [{ months: Math.max(0, claim - current), monthlyContributionBase: currentBase, startOffsetMonths: 0, label: '当前缴费' }];
  }
...
function ensureHistorySegments() {
  ...
  if (!state.historySegments.length) state.historySegments.push({ startYear: '', endYear: '', monthlyContributionBase: '' });
}
function historySegmentsForCalculation() {
  if (state.historyMode !== 'segments' || !Array.isArray(state.historySegments)) return [];
  return state.historySegments
    ...
    .filter(item => item.startYear > 0 && item.endYear >= item.startYear && item.endYear <= NOW.year && item.monthlyContributionBase > 0);
}
function historySegmentRawMonths() {
  return historySegmentsForCalculation().reduce((sum, item) => sum + (item.endYear - item.startYear + 1) * 12, 0);
}
function avgIndexInfo() {
  if (state.historyMode === 'exact') return { value: Math.max(0.01, Number(state.avgIndex || 1)), confidence: 'exact' };
...
function renderIdentity() {
  ...
  stepBody.innerHTML = `
    ...
    ${state.sex === 'female' ? `<div class="field"><label>退休前主要属于哪种情况？</label><div class="choice-stack compact-choices">
      <button type="button" class="choice ${state.femaleCategory === 'worker50' ? 'active' : ''}" data-female-category="worker50"><strong>普通女职工</strong></button>
      <button type="button" class="choice ${state.femaleCategory === 'manager55' ? 'active' : ''}" data-female-category="manager55"><strong>管理 / 技术等岗位</strong></button>
      <button type="button" class="choice ${state.femaleCategory === 'unsure' ? 'active' : ''}" data-female-category="unsure"><strong>不确定</strong></button>
    </div></div>` : ''}`;
...
function retirementModeChoices(r) {
  return `<div class="choice-stack">
    <button type="button" class="choice ${state.retirementMode === 'statutory' ? 'active' : ''}" data-retirement-mode="statutory"><strong>法定退休</strong><span>${ageText(r.statutoryAgeMonths)}</span></button>
    <button type="button" class="choice ${state.retirementMode === 'early' ? 'active' : ''}" data-retirement-mode="early"><strong>提前退休</strong><span>最早 ${ageText(r.earliestAgeMonths)}</span></button>
    <button type="button" class="choice ${state.retirementMode === 'delayed' ? 'active' : ''}" data-retirement-mode="delayed"><strong>延迟退休</strong><span>最晚 ${ageText(r.latestAgeMonths)}</span></button>
  </div>`;
}
function renderPlan() {
  const r = retirement();
  stepTitle.textContent = state.intent === 'flex' ? '以后怎么缴？' : '停止工作后怎么缴？';
  stepDesc.textContent = '';
  const afterStopControls = state.contributionPlan !== 'stop_with_work' ? `
    <div class="field"><label>停止工作后</label><div class="segment"><button type="button" data-after-stop="flex" class="${state.afterStopContributionMode === 'flex' ? 'active' : ''}">灵活就业缴</button><button type="button" data-after-stop="same" class="${state.afterStopContributionMode === 'same' ? 'active' : ''}">按现在基数估</button></div></div>
    ${state.afterStopContributionMode === 'flex' ? `<div class="field"><label>灵活就业月缴费基数（元）</label><input data-key="flexMonthlyContributionBase" type="number" min="0" step="100" value="${state.flexMonthlyContributionBase}"></div>` : ''}` : '';
  stepBody.innerHTML = `
    <div class="field"><label>${state.intent === 'flex' ? '从几岁起按灵活就业规划？' : '几岁停止工作？'}</label><input data-key="stopWorkAge" type="number" min="${Math.floor(currentAgeMonths()/12)}" max="80" step="1" value="${state.stopWorkAge}"></div>
    <div class="field"><label>办理退休</label>${retirementModeChoices(r)}</div>
    ${state.retirementMode !== 'statutory' ? `<div class="field"><label>${state.retirementMode === 'early' ? '提前' : '延后'}</label><div class="segment segment-three">${[12,24,36].map(m => `<button type="button" data-retirement-offset="${m}" class="${Number(state.retirementOffsetMonths) === m ? 'active' : ''}">${m/12}年</button>`).join('')}</div></div>` : ''}
    <div class="field"><label>停止工作后缴多久？</label><div class="choice-stack">
      ...
      <button type="button" class="choice ${state.contributionPlan === 'stop_with_work' ? 'active' : ''}" data-contribution-plan="stop_with_work"><strong>不再缴</strong></button>
    </div></div>
    ${state.contributionPlan === 'actual_months' ? `<div class="field"><label>停止工作后预计还缴</label><div class="number-pair"><div><input data-key="actualFutureYears" type="number" min="0" step="1" value="${state.actualFutureYears}"><span>年</span></div><div><input data-key="actualFutureMonthsExtra" type="number" min="0" max="11" step="1" value="${state.actualFutureMonthsExtra}"><span>个月</span></div></div></div>` : ''}
    ${afterStopControls}`;
...
function historyRowsHtml() {
  ...
  const rows = state.historySegments.map((item, index) => `
    <div class="history-row">
      <div class="history-year"><label>开始年</label><input type="number" min="1950" max="${NOW.year}" data-history-index="${index}" data-history-field="startYear" value="${item.startYear ?? ''}"></div>
      <div class="history-year"><label>结束年</label><input type="number" min="1950" max="${NOW.year}" data-history-index="${index}" data-history-field="endYear" value="${item.endYear ?? ''}"></div>
      <div class="history-base"><label>月缴费基数</label><input type="number" min="0" step="100" data-history-index="${index}" data-history-field="monthlyContributionBase" value="${item.monthlyContributionBase ?? ''}"></div>
  ...
  const raw = historySegmentRawMonths();
  const matchClass = raw && Math.abs(raw - paidMonths()) > 12 ? ' history-total-warn' : '';
  return `${rows}<button type="button" class="small-link history-add" id="historyAddBtn">+ 添加一段</button>${raw ? `<div class="history-total${matchClass}">分段年份约 ${monthsText(raw)} · 累计实际缴费 ${monthsText(paidMonths())}</div>` : ''}`;
}

function renderAmount() {
  stepTitle.textContent = '养老金金额';
  stepDesc.textContent = '';
  stepBody.innerHTML = `
    <div class="field"><label>是否估算金额？</label><div class="segment"><button type="button" data-amount-mode="estimate" class="${state.amountMode === 'estimate' ? 'active' : ''}">估算</button><button type="button" data-amount-mode="skip" class="${state.amountMode === 'skip' ? 'active' : ''}">只看资格</button></div></div>
    ${state.amountMode === 'skip' ? '' : `
      <div class="field"><label>现在的月缴费基数（元）</label><input data-key="monthlyContributionBase" type="number" min="0" step="100" value="${state.monthlyContributionBase}"></div>
      <div class="field"><label>过去的缴费情况</label><div class="choice-stack compact-choices">
        <button type="button" class="choice ${state.historyMode === 'quick' ? 'active' : ''}" data-history-mode="quick"><strong>大概估算</strong></button>
        <button type="button" class="choice ${state.historyMode === 'segments' ? 'active' : ''}" data-history-mode="segments"><strong>按历史缴费基数分段填</strong></button>
        <button type="button" class="choice ${state.historyMode === 'exact' ? 'active' : ''}" data-history-mode="exact"><strong>我有官方平均缴费指数</strong></button>
      </div></div>
      ${state.historyMode === 'quick' ? `<div class="field"><label>大多数年份</label><div class="segment segment-four"><button type="button" data-history-pattern="low" class="${state.historyPattern === 'low' ? 'active' : ''}">较低档</button><button type="button" data-history-pattern="average" class="${state.historyPattern === 'average' ? 'active' : ''}">中间</button><button type="button" data-history-pattern="high" class="${state.historyPattern === 'high' ? 'active' : ''}">较高</button><button type="button" data-history-pattern="unknown" class="${state.historyPattern === 'unknown' || state.historyPattern === 'variable' ? 'active' : ''}">不清楚</button></div></div>` : ''}
      ...
      <details class="disclosure"><summary>当地养老金计发基准</summary><div class="field" style="margin-top:12px"><label>月计发基准（元）</label><input data-key="currentCalcBase" type="number" min="0" step="1" value="${state.currentCalcBase}"></div></details>`}`;
  ...
  stepBody.querySelectorAll('[data-history-mode]').forEach(btn => btn.addEventListener('click', () => {
    state.historyMode = btn.dataset.historyMode;
    if (state.historyMode === 'segments') ensureHistorySegments();
    renderStep();
  }));
  ...
  stepBody.querySelectorAll('[data-history-index]').forEach(input => input.addEventListener('change', () => {
    const index = Number(input.dataset.historyIndex), field = input.dataset.historyField;
    if (!state.historySegments[index]) return;
    state.historySegments[index][field] = input.value === '' ? '' : Number(input.value);
    renderStep();
  }));
  ...
  document.getElementById('historyAddBtn')?.addEventListener('click', () => {
    state.historySegments.push({ startYear: '', endYear: '', monthlyContributionBase: '' });
    renderStep();
  });
}
...
function validateHistorySegments() {
  ...
  const rows = Array.isArray(state.historySegments) ? state.historySegments : [];
  const complete = [];
  for (const item of rows) {
    const start = Number(item.startYear), end = Number(item.endYear), base = Number(item.monthlyContributionBase);
    if (!start && !end && !base) continue;
    if (!(start >= 1950 && start <= NOW.year)) return '请检查历史缴费开始年份。';
    if (!(end >= start && end <= NOW.year)) return '请检查历史缴费结束年份。';
    if (!(base > 0)) return '请填写每一段的月缴费基数。';
    complete.push({ start, end });
  }
  if (!complete.length) return '至少填写一段历史缴费基数。';
  complete.sort((a, b) => a.start - b.start);
  for (let i = 1; i < complete.length; i += 1) {
    if (complete[i].start <= complete[i - 1].end) return '历史缴费时间段不能重叠。';
  }
  return '';
}
...
function renderAgeResult() {
  ...
  if (state.sex === 'female' && state.femaleCategory === 'unsure') {
    main = `<div class="result-hero clean-result"><div class="soft">退休口径未确定</div><div class="result-money">两种可能</div></div><div class="dual-age-grid">${renderAgeResultForCategory('base50','原50岁口径')}${renderAgeResultForCategory('base55','原55岁口径')}</div>`;
  } else {
    const r = retirement(), d = claimDateFromAge(state.birth, r.statutoryAgeMonths);
    main = `<div class="result-hero clean-result"><div class="soft">法定退休年龄</div><div class="result-money">${ageText(r.statutoryAgeMonths)}</div><div class="soft">${d.year}年${d.month}月</div></div>`;
  }
...
function renderContributionStatus(result) {
  ...
  if (result.eligible) return `<div class="decision-card decision-good"><span class="decision-label">缴费资格</span><strong>可以满足最低缴费年限</strong><p>最低 ${required} · 已缴 ${paid} · 未来计划 ${future}</p></div>`;
  return `<div class="decision-card decision-danger"><span class="decision-label">缴费资格</span><strong>还差 ${monthsText(result.plannedContributionShortageMonths)}</strong><p>最低 ${required} · 已缴 ${paid}</p><p>如果连续缴，大约到 ${ageText(continuousQualificationAge(result))} 可累计够。</p></div>`;
}
function renderAmountBlock(result) {
  if (state.amountMode === 'skip') return `<div class="amount-decision"><span>养老金金额</span><strong>未估算</strong></div>`;
  ...
  return `<div class="amount-decision amount-good"><span>预计每月养老金</span><strong>约 ${money(result.pensionCenter)}</strong><p>${money(result.pensionLow)}～${money(result.pensionHigh)} / 月 · ${result.amountConfidence}</p>${notes}</div>`;
}
function segmentSummary(result) {
  if (!result.futureContributionSegments?.length) return '';
  return result.futureContributionSegments.map(s => `${s.label || '未来缴费'} ${monthsText(s.months)}，基数约 ${money(s.monthlyContributionBase)}`).join('；');
}
...
function renderResult() {
  ...
  resultView.innerHTML = `<div class="result-hero clean-result"><div class="soft">退休计划</div><div class="result-money">${ageText(claim)}办理退休</div><div class="soft">${d.year}年${d.month}月</div><div class="result-grid"><div class="result-cell"><div class="k">停止工作</div><div class="v">${ageText(stop)}</div></div><div class="result-cell"><div class="k">已缴费</div><div class="v">${monthsText(result.paidMonths)}</div></div><div class="result-cell"><div class="k">未来缴费</div><div class="v">${monthsText(result.futureContributionMonths)}</div></div><div class="result-cell"><div class="k">最低要求</div><div class="v">${monthsText(result.requiredContributionMonths)}</div></div></div></div>
    ${renderContributionStatus(result)}
    ${segments ? `<div class="plain-note section"><strong>未来缴费</strong><span>${segments}</span></div>` : ''}
    <div class="card section"><h2>养老金金额</h2>${renderAmountBlock(result)}</div>
    <div class="card section"><h2>时间线</h2><div class="timeline modern-timeline">${timelineItem('现在', `已缴 ${monthsText(result.paidMonths)}`)}${state.intent === 'normal' ? '' : timelineItem(ageText(stop), '停止工作')}${timelineItem(ageText(claim), `${d.year}年${d.month}月退休`)}</div></div>
    <div class="card section"><p class="source-box compact-copy">${PENSION_SCOPE} · ${POLICY_VERSION} · 最终待遇以参保地经办核定为准。</p></div>
...
if (saved) {
  ...
  const resumeText = document.getElementById('resumeText'); if (resumeText) resumeText.textContent = '上次的规划还在。';
