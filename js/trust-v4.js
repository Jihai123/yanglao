import {
  ...
  getRegionV4,
} from './sources-v4.js?v=20260829-v4';
...
function sourceLink(item, label = '查看官方依据') {
  if (!item?.url) return '';
...
function renderHomeTrust() {
  ...
  slot.innerHTML = `<div class="trust-strip"><div><span class="trust-dot"></span><strong>政策来源已核验</strong><span>${DATA_VERIFIED_AT}${days ? ` · ${days}天前` : ''}</span></div>${sourceLink(rules, '政策依据')}</div>`;
}
